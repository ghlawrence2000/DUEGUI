...
{
char temp[60];
st.toCharArray(temp,60);
//printf("DUEGUI::getDisplayXSize() : %d \nDUEGUI::getDisplayYSize() : %d \n", getDisplayXSize() , getDisplayYSize()) ;
//printf("DUEGUI::Put_Text(%s , %d, %d, %d )\n", temp, x, y, font_number);
...
//	touch_disp_y_size		= CAL_Y_SIZE;
	...
	touch_disp_x_size		= 320;//getDisplayXSize();
	touch_disp_y_size		= 240;//getDisplayYSize();
...
/* 
  ...
  if (_revTouchX) c=touch_disp_x_size-c; 
  if (c<0){c=0;}
  if (c>touch_disp_x_size){c=touch_disp_x_size;};
  printf("touch x %ld\n",c);
...
int DUEGUI::addPanel(word x,word y,word xs,word ys,long colour,long borcolour,long textcolour,byte borwidth,String top,int xo,int yo,int font,bool visible,int URN){
  ...
  if (xo==posRight){
    ...
    if (xo==posCentre){
      GUIobject_xo[GUI_objects]=(xs-(top.length()*16))/2;
    } else {
      if (xo==posLeft){
  ...
  if (yo==posCentre){
    GUIobject_yo[GUI_objects]=((ys-16)/2)   ;
//    GUIobject_yo[GUI_objects]=((ys-(cfont.y_size+(borwidth*2))/2)+y)    ;
  } else {
